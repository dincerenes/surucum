...
-- denetimi yapar. Denetim atlanırsa fonksiyon tüm sürücü verisini herkese
-- açar — bu yüzden `perform public.admin_assert(...)` satırı her gövdenin
-- ilk satırıdır ve öyle kalmalıdır.
--
-- Toplamlar neden Postgres'te hesaplanıyor: alternatif, her sürücünün her
-- seferini tarayıcıya indirip orada saymaktı. Bin sürücü × on bin sefer,
-- bir sayı üretmek için on milyon satır demek. Toplam sunucuda çıkar,
-- tele bir sayı biner.

-- ---------------------------------------------------------------------------
-- Yetki ve yardımcılar (istemciye kapalı)
-- ---------------------------------------------------------------------------
...
begin
  if not exists (
    ...
    -- genel bir hata kodu 500 dönerdi ve panel "sunucu çöktü" derdi.
 ...
 * istemciden GELMEZ; yalnızca aşağıdaki admin_system_health() gövdesinde,
 * bu migration'da yazılmış sabit metinlerle çağrılır. EXECUTE yetkisi tüm
 * istemci rollerinden geri alınıyor: dışarıdan çağrılabilseydi keyfi
 * sorgu çalıştırma açığı olurdu.
 *
 * Sayım ve örnekleme ayrı iki sorgu: tek sorguda array_agg kullanılsaydı
 * milyonlarca kimlik belleğe toplanırdı. Örnek sorgusu limit 5 ile bağlı.
 */
 ...
 * sürücü aktiflik grafiğini geleceğe veya geçmişe kaydırırdı. Sunucu saati
 * tek referans — bu, senkron imlecinin de dayandığı kural.
 */
...
begin
  ...
  return jsonb_build_object(
    ...
    -- Senkron gerçekten akıyor mu: son 24 saatte buluta düşen satır sayısı.
 ...
 * değil: panel Türkiye'den okunuyor ve "dün" derken kastedilen yerel gün.
 * UTC kullanılsaydı gece 03:00'te girilen sefer bir önceki güne düşerdi.
 *
 * Sefer ve net için business_date kullanılıyor (sürücünün iş günü),
 * kayıt ve aktiflik için sunucu zamanı. İkisi kasıtlı olarak farklı:
 * biri sürücünün gününü, diğeri sistemin gününü ölçüyor.
 */
...
/*
 * Kullanıcı listesi. Bu fonksiyon PARA TOPLAMI döner ama seferin kendisini
 * DÖNMEZ — liste ekranında kimin ne kazandığını görmek operasyon için
 * yeterli, hangi seferi ne zaman yaptığını görmek değil.
 *
 * Erişim burada loglanmıyor: liste her sayfa yenilemede çağrılır ve her
 * çağrıyı loglamak denetim kaydını gürültüye boğardı. Ham veriye inen
 * fonksiyonlar (admin_user_detail, admin_user_records) loglanıyor.
 */
...
-- ---------------------------------------------------------------------------
-- Kullanıcı detayı — LOGLANIR
-- ---------------------------------------------------------------------------

/*
 * Tek kullanıcının toplamları.
 *
 * Bu fonksiyon VOLATILE'dır (stable değil) çünkü denetim kaydına satır
 * yazıyor. `stable` işaretlenseydi Postgres yazmaya izin vermezdi.
 */
...
-- ---------------------------------------------------------------------------
-- Ham kayıtlar — LOGLANIR, 'support' rolüne KAPALI
-- ---------------------------------------------------------------------------
 ...
 *
 * Üç koruma var:
 *  1. Rol denetimi — 'support' buraya giremez, yalnızca toplam görür.
 *  2. Tablo adı beyaz listeden geçer. Dinamik SQL'de tablo adı
 *     parametreleştirilemez (bind edilemez), bu yüzden tek güvenli yol
 *     sabit bir listeye üyelik kontrolü.
 *  3. Her çağrı denetim kaydına düşer — kim, kimin, hangi tablosuna baktı.
 *
 * Sıralama her tabloda server_updated_at: on bir tablonun ortak sütunu bu
 * ve sunucu saati tek güvenilir referans.
 */
 ...
 *
 * Buradaki denetimler genel değil, BU uygulamaya özel. Her biri gerçek bir
 * hata sınıfını yakalıyor:
 *
 *  - brüt ≠ komisyon + net  → para hesabında kayma; üçü de ayrı saklandığı
 *                             için sessizce bozulabilir
 *  - yetim yabancı anahtar  → senkron sırası bozulmuş; çocuk kayıt
 *                             ebeveyninden önce gitmiş
 *  - business_date kayması  → gün kesme saati hesabı hatalı; gece
 *                             vardiyası yanlış güne yazılıyor
 *  - gelecek tarihli kayıt  → cihaz saati yanlış
 *  - açık kalmış vardiya    → sürücü vardiyayı kapatmayı unutmuş;
 *                             TL/saat paydası bozulur
 */
...
begin
  ...
  return jsonb_build_object(
    ...
    -- Yakıt fiyatı tazeliği: cron'un gerçekten çalışıp çalışmadığının tek
    -- göstergesi. Fiyat bayatladıysa çevrimdışı maliyet tahmini de bayat.
 ...
 * yok — olması da istenmez — bu yüzden e-posta çözümlemesi burada yapılıyor.
 * Denetim kaydı admin_users üzerindeki tetikleyiciden otomatik düşer.
 */
 ...
 * Yönetici listesi.
 *
 * admin_users tablosu yalnızca user_id tutuyor; e-posta auth.users'ta ve
 * istemcinin o tabloya erişimi yok — olmamalı da. Bu fonksiyon iki tarafı
 * sunucuda birleştiriyor, panelin auth şemasına hiç dokunması gerekmiyor.
 */
