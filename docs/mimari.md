# Sürücüm — mimari ve kurallar

Kodun **neden böyle** olduğu, ürün kararları ve depo kuralları. Kısa
tanıtım için [README](../README.md).
...
06:00'da başlayıp ertesi 06:00'da bittiği için cuma gecesi 23:00–02:00
...
noktalar; yalnızca ad ve "Şu an aktif" / "Bugün 17:00–20:00" etiketi —
ilçe, saat listesi ve açıklama satırları sürücünün isteğiyle kaldırıldı).
Sezonu geçmiş bölge (kışın deniz kıyısı gibi) gizleniyor.

Yoğun saatler **zirve zirve** bulunuyor: her yerel zirve (günün en
yükseğinin en az %65'i) kendi penceresini açıyor ve kendi zirvesinin
%80'inin üstünde kalan komşu saatlere en fazla 4 saat büyüyor. Tek bir
gün eşiği hafta içi sabah zirvesini ve cuma gecesini yutuyordu.
...
endeksleri, belediye açık verisi). Her kartın üstünde "Tahmini"
yazıyor; hiçbir yerde sayı ya da yüzde gösterilmiyor. Ayrı bir "Bu
bilgiler nereden?" notu vardı, sürücünün isteğiyle kaldırıldı.
...
### Gerçek iPhone'a kurmak

Xcode'dan: `ios/Srcm.xcworkspace` → Srcm → Signing & Capabilities →
**Team** (Apple hesabı) → Product → Scheme → Edit Scheme → Run →
**Release** (Mac'e bağlı olmadan çalışsın) → cihazı seç → ▶. İlk
kurulumda iPhone'da Ayarlar → Genel → VPN ve Aygıt Yönetimi → **Güven**.
Ücretsiz Apple hesabıyla kurulan uygulama 7 günde bir yeniden
kurulmalı. `expo prebuild` `ios/` klasörünü yeniden ürettiğinde takım
ve Release seçimi sıfırlanıyor.

**iOS 27 sahne yaşam döngüsü:** iOS 27 bunu benimsemeyen uygulamayı
açılışta durduruyor (simülatördeki iOS 26'da görünmüyor). Expo'nun
prebuild şablonu henüz eski düzende; `plugins/with-scene-lifecycle.js`
(app.json eklentisi) her prebuild'de Info.plist'e sahne yapılandırmasını
ekliyor ve AppDelegate'in pencereyi kendisi açmasını kaldırıyor. Şablon
güncellenince eklenti kaldırılabilir.
